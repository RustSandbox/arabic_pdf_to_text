...
impl GeminiClient {
    ...
    async fn upload_file(&self, pdf_data: &[u8], display_name: &str) -> Result<String> {
        ...
        println!("\n{} {}", 
            "🔐".cyan(), 
            format!("Initiating secure file transfer for: {}", display_name).cyan().bold()
        );
        println!("  {} File size: {} bytes", "📊".yellow(), num_bytes.to_string().green());
        ...
        headers.insert("X-Goog-Upload-Header-Content-Length", HeaderValue::from_str(&num_bytes.to_string())?);  
        headers.insert("X-Goog-Upload-Header-Content-Type", HeaderValue::from_static("application/pdf"));
        ...
        upload_headers.insert(CONTENT_LENGTH, HeaderValue::from_str(&num_bytes.to_string())?);
        ...
        let file_info: FileUploadResponse = upload_response.json().await
        ...
        println!("  {} Upload ID: {}", "🆔".cyan(), file_info.file.name.yellow());
        println!("  {} Cloud URI: {}", "☁️".cyan(), file_info.file.uri.blue().underline());
        println!("  {} Status: {}", "✅".green(), "Ready for AI processing".green());
    ...
    pub async fn extract_page_range(&self, file_uri: &str, start_page: usize, end_page: usize) -> Result<String> {
        ...
        if !response.status().is_success() {
            ...
            anyhow::bail!("Gemini API error for pages {}-{}: {}", start_page, end_page, error_text);
        }
        
        let gemini_response: GeminiResponse = response.json().await
            .context(format!("Failed to parse response for pages {}-{}", start_page, end_page))?;
            
        gemini_response.candidates
            .first()
            .and_then(|c| c.content.parts.first())
            .map(|p| p.text.clone())
            .ok_or_else(|| anyhow::anyhow!("No text found in response for pages {}-{}", start_page, end_page))
    }
...
mod tests {
    ...
    async fn test_gemini_client_success() {
        ...
        let upload_init_mock = server.mock("POST", "/upload/v1beta/files")
        ...
        let upload_mock = server.mock("POST", "/upload/123")
            .with_status(200)
        ...
        let generate_mock = server.mock("POST", "/v1beta/models/gemini-2.5-flash:generateContent")
            .match_query(mockito::Matcher::Regex("key=test_key".to_string()))
            .with_status(200)
            .with_body(r#"{
                "candidates": [{
