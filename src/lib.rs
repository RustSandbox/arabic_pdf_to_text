...
pub async fn process_pdf(path: &str, config: &Config) -> Result<String> {
    ...
    let tasks: Vec<_> = (0..num_chunks)
        .map(|chunk_idx| {
            ...
            tokio::spawn(async move {
                ...
                let mut retries = 3;
                let mut result = client.extract_page_range(&file_uri, start_page, end_page).await;
                ...
                while retries > 0 && result.is_err() {
                    if let Err(e) = &result {
                        if e.to_string().contains("RESOURCE_EXHAUSTED") {
                            ...
                            retries -= 1;
                            result = client.extract_page_range(&file_uri, start_page, end_page).await;
                        } else {
                            break;
                        }
                    }
    ...
    let success_count = results.iter().filter(|s| !s.is_empty() && !s.starts_with("[Pages")).count();
